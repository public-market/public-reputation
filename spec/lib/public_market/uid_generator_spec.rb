...
describe PublicMarket::UIDGenerator do
  let(:uid_generator) { described_class.new(options) }
  ...
  shared_examples_for 'duplicate without length increment' do
    it 'sets permalink field' do
      expect { subject }.to change(resource, :uid).from(nil).to(String)
      expect(resource.uid).to match(regex)
    end
  end
  ...
  shared_examples_for 'generating permalink' do
    ...
    context 'and generated candidate is NOT unique' do
      ...
      context 'and more than half of the value space is taken' do
        ...
        it 'sets permalink field' do
          expect { subject }.to change(resource, :uid).from(nil).to(String)
          expect(resource.uid).to match(regex_more_than_half)
        end
      end
    end
  end
  ...
  describe '#included' do
    context 'generates .uid_generator on host' do
      it 'returns uid generator' do
        expect(model.uid_generator).to be(uid_generator)
      end
    end
    ...
    context 'generates validation hooks on host' do
      ...
      context 'when permalink field value is present' do
        let(:resource) { model.new(uid: 'Test') }

        it 'does not touch field' do
          expect { subject }.not_to change(resource, :uid)
        end
      end
    end
  end
